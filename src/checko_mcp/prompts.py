...
Перед началом учти правила из ресурса `checko://docs/agent-guide` и проверь \
`meta.balance` после первого ответа (предупреди пользователя при балансе < 100 руб).

Последовательность вызовов:

1. Если `{query}` — название (не ИНН/ОГРН), сначала вызови `search(by="name", obj="org", query="{query}")`.
   Если кандидатов несколько — уточни у пользователя, какую организацию проверять.
   Если `{query}` уже выглядит как ОГРН/ИНН — этот шаг пропусти и сразу зови `get_company`.
2. `get_company(ogrn=<ОГРН>)` — статус, реквизиты, руководство, учредители.
   Зафиксируй факторы риска: `Санкции`, `НелегалФин`, `НедобПост`, `МассРуковод`,
   `МассУчред`, `МассАдрес`, `Обрем`.
3. `get_finances(ogrn=<ОГРН>, extended=true)` — динамика по строкам:
   2110 (выручка), 2400 (чистая прибыль), 1300 (капитал — отрицательный = красный флаг),
   1400/1500 (долговая нагрузка). Бери последние 3–5 лет.
4. `get_legal_cases(ogrn=<ОГРН>, role="defendant", actual=true, active=true)` —
   активные иски-ответчик. Зафиксируй `ОбщСуммИск`, ключевых истцов, число дел.
5. `get_fedresurs(ogrn=<ОГРН>)` — ищи `CreditorIntentionGoToCourt` (намерение о банкротстве),
   `PledgeAgreement` (залоги).
6. `get_bankruptcy_messages(ogrn=<ОГРН>)` — открытые банкротные процедуры.

Сформируй структурированное резюме:

- **Общий вывод:** ✅ работать можно / ⚠️ работать с осторожностью / 🚫 не рекомендуется.
- **Базовые факты:** статус, размер (МСП-категория), руководитель, ключевые учредители.
- **Финансы:** выручка/прибыль за последние годы с динамикой (рост или падение в %).
- **Активные иски:** число, общая сумма, крупнейшие истцы.
- **Сигналы рисков:** список с приоритетом (🔴/🟠/🟡) и со ссылкой на конкретное поле/факт.
- **Источники:** на каких данных и какой давности построено заключение.
"""
...
def _verify_bank_details(args: dict[str, Any]) -> str:
    ...
    subject_tool = "get_company" if len(inn) == 10 else "get_entrepreneur"
    return f"""\
...
1. `{subject_tool}(inn="{inn}")` — проверь, что субъект существует и его `Статус.Наим == "Действует"`.
...
def _assess_bankruptcy_risk(args: dict[str, Any]) -> str:
    ...
    base_tool = (
        "get_company" if len(inn) == 10
        else "get_entrepreneur (а если не найдено — get_person)"
    )
    return f"""\
...
1. {base_tool}(inn="{inn}") — статус, базовые реквизиты, факторы риска.
2. `get_fedresurs(inn="{inn}")` — ищи сообщения `CreditorIntentionGoToCourt` (намерения \
кредиторов подать на банкротство) и `BankruptcyMessage` (публикации в рамках процедур).
   Зафиксируй даты и кредиторов.
3. `get_bankruptcy_messages(inn="{inn}")` — есть ли уже открытые банкротные процедуры.
4. `get_legal_cases(inn="{inn}", role="defendant", active=true, actual=true)` — \
активные иски-ответчик. Сумма исков и количество.
5. Если юрлицо/ИП — `get_finances(inn="{inn}", extended=true)`: отрицательный капитал \
(стр. 1300), снижение выручки (стр. 2110), рост обязательств (стр. 1400+1500).
...
1. **Базовый профиль** — `get_person(inn="{inn}")`. Зафиксируй:
...
2. **По каждой связанной организации** — `get_company(ogrn=<ОГРН>)`. Особое внимание:
   статус, факторы риска, признаки реальной деятельности (выручка, налоги, лицензии).

3. **По каждому ИП** — `get_entrepreneur(ogrnip=<ОГРНИП>)`. Статус, виды деятельности, лицензии.
...
1. Если `{query}` — название, сначала `search(by="name", obj="org", query="{query}")`.
   Если уже есть ОГРН/ИНН — этот шаг пропусти.
2. `get_company(ogrn=<ОГРН>)` — статус, руководство, учредители. Особое внимание:
